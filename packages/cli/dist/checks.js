...
exports.checkVsockSupport = checkVsockSupport;
exports.checkBuildxAvailable = checkBuildxAvailable;
exports.checkLaneBuilder = checkLaneBuilder;
exports.checkOciExportSupport = checkOciExportSupport;
exports.requireOciExportSupport = requireOciExportSupport;
exports.checkRiscv64Support = checkRiscv64Support;
...
function checkVsockSupport() {
    ...
    try {
        ...
        const result = (0, child_process_1.execSync)("docker run --rm --privileged alpine:latest ls -la /dev/vsock", {
            encoding: "utf8",
            stdio: "pipe",
        });
        if (result.includes("/dev/vsock")) {
            ...
            console.error("vsock support is required for LaneLayer to function properly.");
...
function checkLaneBuilder() {
    // Check if current builder supports OCI exports
    checkOciExportSupport();
}
function checkOciExportSupport() {
    try {
        // Get all available builders
        const buildersList = (0, child_process_1.execSync)("docker buildx ls", {
            encoding: "utf8",
            stdio: "pipe",
        });
        // Get current builder info
        const currentBuilderInfo = (0, child_process_1.execSync)("docker buildx inspect --bootstrap", {
            encoding: "utf8",
            stdio: "pipe",
        });
        ...
        if (currentBuilderInfo.includes("Driver: docker")) {
            ...
            const compatibleBuilders = [];
            for (const line of lines) {
                if (line.includes("docker-container") ||
...
function requireOciExportSupport() {
    try {
        // Get all available builders
        const buildersList = (0, child_process_1.execSync)("docker buildx ls", {
            encoding: "utf8",
...
function checkRiscv64Support() {
    try {
        ...
        (0, child_process_1.execSync)("docker run --rm --platform linux/riscv64 hello-world:latest", {
            stdio: "pipe",
        });
    }
    catch (err) {
        ...
        try {
            (0, child_process_1.execSync)("docker run --privileged --rm tonistiigi/binfmt --install riscv64", { stdio: "inherit" });
