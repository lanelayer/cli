...
exports.handleBuildCommand = handleBuildCommand;
exports.handleUpCommand = handleUpCommand;
exports.buildImage = buildImage;
exports.runDevEnvironment = runDevEnvironment;
...
const checks_2 = require("../checks");
const generate_1 = require("../generate");
// Import tar context builder
const tar_context_1 = require("../tar-context");
...
function needsProfileChange(currentProfile, requestedProfile) {
    if (!currentProfile) {
...
function hasHotReloadSupport(imageTag) {
    try {
        const label = (0, child_process_1.execSync)(`docker inspect ${imageTag} --format '{{index .Config.Labels "lane.hot-reload"}}'`, {
            encoding: "utf8",
...
function handleBuildCommand(args) {
    ...
    if (args.includes("--help") || args.includes("-h")) {
        (0, help_1.showCommandHelp)("build");
        return;
    }
    (0, checks_1.checkBuildxAvailable)();
    (0, checks_1.checkLaneBuilder)();
    ...
    let useTarContext = undefined; // Will set default after parsing profile
    let turbo = false; // Enable multi-core QEMU for stage profiles
    let guestAgentImage; // Guest agent image for prod/prod-debug profiles
    let hot = false; // Enable hot reloading for dev profile
    let useExistingImage; // Use existing Docker image instead of building
    ...
    if (!useDepot && !noDepot) {
        const depotJsonPath = (0, path_1.join)((0, process_1.cwd)(), "depot.json");
        if ((0, fs_1.existsSync)(depotJsonPath)) {
            useDepot = true;
...
function handleUpCommand(args) {
    // Check for help flag
    if (args.includes("--help") || args.includes("-h")) {
        (0, help_1.showCommandHelp)("up");
        return;
    }
    (0, checks_1.checkBuildxAvailable)();
    (0, checks_1.checkLaneBuilder)();
...
function resolvePlatforms(profile) {
    switch (profile) {
        case "dev":
...
function buildLinuxKitImage(yamlPath, profile, ociTarPath, cacheDir, forceRebuild = false) {
    console.log("Building LinuxKit image...");
    if (ociTarPath) {
        console.log(`Using OCI image: ${ociTarPath}`);
    }
    const currentDir = (0, process_1.cwd)();
    const imageName = process.env.LINUXKIT_IMAGE || constants_1.LANE_SNAPSHOT_BUILDER_IMAGE;
    // Get current user UID/GID for Docker commands
    const uid = process.getuid?.() ?? 0;
    const gid = process.getgid?.() ?? 0;
    // Check if Docker config exists for authentication
    const dockerConfigPath = (0, path_1.join)((0, os_1.homedir)(), ".docker", "config.json");
    let dockerConfigMount = [];
    if ((0, fs_1.existsSync)(dockerConfigPath)) {
        try {
            const configContent = (0, fs_1.readFileSync)(dockerConfigPath, "utf8");
            ...
            if (config.credsStore === "desktop") {
                ...
                dockerConfigMount = [
                    "-v",
                    `${dockerConfigPath}:/cache/.docker/config.json:ro`,
                ];
    ...
    try {
        ...
        const vcTarPath = cacheDir
            ? (0, path_1.join)(cacheDir, `vc${debugSuffix}.tar`)
            : (0, path_1.join)(currentDir, `vc${debugSuffix}.tar`);
        const vcSquashfsPath = cacheDir
            ? (0, path_1.join)(cacheDir, `vc${debugSuffix}.squashfs`)
            : (0, path_1.join)(currentDir, `vc${debugSuffix}.squashfs`);
        if (!forceRebuild && (0, fs_1.existsSync)(vcSquashfsPath)) {
            console.log(`✅ vc${debugSuffix}.squashfs already exists, skipping LinuxKit build and squashfs creation`);
        }
        else {
            if (forceRebuild && (0, fs_1.existsSync)(vcTarPath)) {
                console.log(`🔄 Force rebuild: removing existing vc${debugSuffix}.tar`);
                (0, fs_1.unlinkSync)(vcTarPath);
            }
            // Ensure .moby directory exists in cache
            if (cacheDir) {
                const mobyDir = (0, path_1.join)(cacheDir, ".moby");
                if (!(0, fs_1.existsSync)(mobyDir)) {
                    (0, fs_1.mkdirSync)(mobyDir, { recursive: true });
                }
            }
            // Ensure LinuxKit cache directory exists
            const linuxkitCacheDir = (0, path_1.join)((0, os_1.homedir)(), ".cache", "lane", "linuxkit-cache");
            if (!(0, fs_1.existsSync)(linuxkitCacheDir)) {
                (0, fs_1.mkdirSync)(linuxkitCacheDir, { recursive: true });
            }
            // Import OCI image into LinuxKit cache if provided
            if (ociTarPath && (0, fs_1.existsSync)(ociTarPath)) {
                ...
                const importCommand = [
                    ...
                    `${cacheDir}:/cache`,
                    "-v",
                    `${linuxkitCacheDir}:/home/user/.linuxkit/cache`,
                    ...dockerConfigMount,
                    "-w",
                    "/cache",
                    imageName,
                ...
                (0, child_process_1.execSync)(importCommand.join(" "), {
                    stdio: "inherit",
                    cwd: currentDir,
                });
            ...
            (0, child_process_1.execSync)(command.join(" "), { stdio: "inherit", cwd: currentDir });
            console.log("✅ LinuxKit image built successfully");
            // Print SHA256 of vc.tar before it gets consumed
            try {
                if ((0, fs_1.existsSync)(vcTarPath)) {
                    const vcTarHash = (0, child_process_1.execSync)(`sha256sum "${vcTarPath}"`, {
                        encoding: "utf8",
                    })
                        .trim()
                        .split(" ")[0];
            ...
            const snapshotCommand = [
                ...
                "-w",
                "/cache",
                constants_1.LANE_SNAPSHOT_BUILDER_IMAGE,
                "bash",
                "-c",
            ...
            const result = (0, child_process_1.spawnSync)(snapshotCommand[0], snapshotCommand.slice(1), {
        ...
        if (profile === "prod" || profile === "prod-debug") {
            const cmSnapshotPath = cacheDir
                ? (0, path_1.join)(cacheDir, `vc-cm-snapshot${debugSuffix}`)
                : (0, path_1.join)(currentDir, `vc-cm-snapshot${debugSuffix}`);
            const cmSquashfsPath = cacheDir
                ? (0, path_1.join)(cacheDir, `vc-cm-snapshot${debugSuffix}.squashfs`)
                : (0, path_1.join)(currentDir, `vc-cm-snapshot${debugSuffix}.squashfs`);
            const verityPath = cacheDir
                ? (0, path_1.join)(cacheDir, `vc-cm-snapshot${debugSuffix}.squashfs.verity`)
                : (0, path_1.join)(currentDir, `vc-cm-snapshot${debugSuffix}.squashfs.verity`);
            const rootHashPath = cacheDir
                ? (0, path_1.join)(cacheDir, `vc-cm-snapshot${debugSuffix}.squashfs.root-hash`)
                : (0, path_1.join)(currentDir, `vc-cm-snapshot${debugSuffix}.squashfs.root-hash`);
            // Check if we need to create Cartesi machine snapshot
            if (!forceRebuild && (0, fs_1.existsSync)(cmSnapshotPath)) {
                ...
                if (forceRebuild && (0, fs_1.existsSync)(cmSnapshotPath)) {
                    ...
                    (0, child_process_1.execSync)(`rm -rf "${cmSnapshotPath}"`, { stdio: "ignore" });
                }
                ...
                const cartesiResult = (0, child_process_1.spawnSync)(cartesiCommand[0], cartesiCommand.slice(1), { stdio: "inherit", cwd: currentDir });
                ...
                try {
                    const hashPath = (0, path_1.join)(cmSnapshotPath, "hash");
                    if ((0, fs_1.existsSync)(hashPath)) {
                        const hashBuffer = (0, fs_1.readFileSync)(hashPath);
                        ...
                        try {
                            if ((0, fs_1.existsSync)(cmSnapshotPath)) {
                                const contents = (0, child_process_1.execSync)(`ls -la "${cmSnapshotPath}"`, {
                                    encoding: "utf8",
                                });
            ...
            if (!forceRebuild && (0, fs_1.existsSync)(cmSquashfsPath)) {
                ...
                const compressResult = (0, child_process_1.spawnSync)(compressCommand[0], compressCommand.slice(1), { stdio: ["inherit", "pipe", "pipe"], cwd: currentDir });
                if (compressResult.status !== 0) {
                    console.error("Compression command failed with output:");
                    if (compressResult.stdout)
                        console.error("stdout:", compressResult.stdout.toString());
                    if (compressResult.stderr)
                        console.error("stderr:", compressResult.stderr.toString());
            ...
            let fileSize;
            try {
                const stats = (0, fs_1.statSync)(cmSquashfsPath);
                fileSize = stats.size;
                if (fileSize % 512 !== 0) {
            ...
            if (!forceRebuild && (0, fs_1.existsSync)(verityPath) && (0, fs_1.existsSync)(rootHashPath)) {
                ...
                const verityResult = (0, child_process_1.spawnSync)(verityCommand[0], verityCommand.slice(1), { stdio: ["inherit", "pipe", "pipe"], cwd: currentDir });
                if (verityResult.status !== 0) {
                    console.error("Verity setup command failed with output:");
                    if (verityResult.stdout)
                        console.error("stdout:", verityResult.stdout.toString());
                    if (verityResult.stderr)
                        console.error("stderr:", verityResult.stderr.toString());
                ...
                const verifyResult = (0, child_process_1.spawnSync)(verifyCommand[0], verifyCommand.slice(1), { stdio: ["inherit", "pipe", "pipe"], cwd: currentDir });
                if (verifyResult.status !== 0) {
                    console.error("Verity verification failed with output:");
                    if (verifyResult.stdout)
                        console.error("stdout:", verifyResult.stdout.toString());
                    if (verifyResult.stderr)
                        console.error("stderr:", verifyResult.stderr.toString());
            ...
            try {
                const vcSquashfsPath = cacheDir
                    ? (0, path_1.join)(cacheDir, `vc${debugSuffix}.squashfs`)
                    : (0, path_1.join)(currentDir, `vc${debugSuffix}.squashfs`);
                if ((0, fs_1.existsSync)(vcSquashfsPath)) {
                    const vcSquashfsHash = (0, child_process_1.execSync)(`sha256sum "${vcSquashfsPath}"`, {
                        encoding: "utf8",
                    })
                        .trim()
                        .split(" ")[0];
            ...
            try {
                const cmSquashfsPath = cacheDir
                    ? (0, path_1.join)(cacheDir, `vc-cm-snapshot${debugSuffix}.squashfs`)
                    : (0, path_1.join)(currentDir, `vc-cm-snapshot${debugSuffix}.squashfs`);
                if ((0, fs_1.existsSync)(cmSquashfsPath)) {
                    const cmSquashfsHash = (0, child_process_1.execSync)(`sha256sum "${cmSquashfsPath}"`, {
                        encoding: "utf8",
                    })
                        .trim()
                        .split(" ")[0];
            ...
            try {
                if ((0, fs_1.existsSync)(rootHashPath)) {
                    const rootHash = (0, child_process_1.execSync)(`cat "${rootHashPath}"`, {
                        encoding: "utf8",
                    }).trim();
...
function buildImage(imageTag, profile, userCacheDir, forceRebuild = false, useDepot = false, useTarContext = true, forceDockerTar = false, turbo = false, guestAgentImage, hot = false, useExistingImage) {
    const currentDir = (0, process_1.cwd)();
    ...
    if (useExistingImage) {
        ...
        if (profile === "stage" ||
            ...
            const yamlPath = (0, generate_1.generateLinuxKitYaml)(imageTag, profile, cacheDir, undefined, guestAgentImage);
    ...
    if (useDepot) {
        ...
        try {
            (0, child_process_1.execSync)("depot --version", { stdio: "ignore" });
    ...
    if (!(0, fs_1.existsSync)((0, path_1.join)(currentDir, "Dockerfile"))) {
    ...
    if ((profile === "prod" || profile === "prod-debug") && guestAgentImage) {
        const guestAgentCacheFile = (0, path_1.join)(cacheDir, "guest-agent-image.txt");
        let needsGuestAgentRebuild = false;
        if ((0, fs_1.existsSync)(guestAgentCacheFile)) {
            const cachedImage = (0, fs_1.readFileSync)(guestAgentCacheFile, "utf8").trim();
        ...
        (0, fs_1.writeFileSync)(guestAgentCacheFile, guestAgentImage);
    }
    // Create OCI tar file path - always use cache directory
    const safeImageName = imageTag.replace(/[:/]/g, "-");
    const ociTarPath = (0, path_1.join)(cacheDir, `${safeImageName}.tar`);
    let contextBuilder;
    let contextTarPath;
    if (useTarContext) {
        // Create deterministic tar context
        contextBuilder = new tar_context_1.TarContextBuilder({
            contextPath: currentDir,
            outputPath: (0, path_1.join)(cacheDir, "build-context.tar"),
            deterministic: true,
    ...
    try {
        ...
        if (useStdin && contextTarPath) {
            ...
            const tarContent = (0, fs_1.readFileSync)(contextTarPath);
        ...
        if (profile === "stage" ||
            ...
            profile === "prod-debug") {
            console.log(`\n🔄 Building LinuxKit image for ${profile} profile...`);
            const yamlPath = (0, generate_1.generateLinuxKitYaml)(imageTag, profile, cacheDir, ociTarPath, guestAgentImage);
...
function runDevEnvironment(imageTag, profile, cacheDir, forceRebuild = false, forceRestart = false, useDepot = false, useTarContext = true, forceDockerTar = false, turbo = false, guestAgentImage, hot = false, useExistingImage) {
    ...
    try {
        ...
        try {
            ...
            const allContainers = (0, child_process_1.execSync)('docker ps --format "{{.Names}}:{{.Ports}}"', { encoding: "utf8" }).trim();
            const pathHash = (0, cli_1.getPathHash)();
            // Check for containers using port 8080 that aren't ours
            const containersUsing8080 = allContainers
                .split("\n")
                .filter((line) => line.includes(":8080") && !line.startsWith(`${pathHash}-lane-`));
            if (containersUsing8080.length > 0) {
                ...
                console.error("You can stop all LaneLayer environments with: lane prune --local");
                process.exit(1);
            }
        }
        catch (err) {
        ...
        if (!forceRestart && (0, fs_1.existsSync)(composePath) && !profileChangeNeeded) {
            try {
                const composeContent = (0, fs_1.readFileSync)(composePath, "utf8");
        ...
        if (needsUpdate) {
            ...
            (0, generate_1.generateDockerCompose)(imageTag, profile, ociTarPath, buildCacheDir, turbo, guestAgentImage, profile === "dev" && hot);
        ...
        if (needsUpdate) {
            ...
            const containersExist = (0, fs_1.existsSync)(composePath) &&
                (0, child_process_1.execSync)(`docker compose -f ${composePath} ps --services --filter "status=running"`, { encoding: "utf8" }).trim().length > 0;
            if (containersExist) {
                // Containers exist, force recreate only the isolated_service
                console.log("Force recreating isolated_service...");
                (0, child_process_1.execSync)(`docker compose -f ${composePath} up -d --force-recreate --wait isolated_service`, { stdio: "inherit" });
            }
            else {
                // First startup, just start all services normally
                console.log("Starting all services...");
                (0, child_process_1.execSync)(`docker compose -f ${composePath} up -d --wait`, {
                    stdio: "inherit",
                });
            }
        }
        ...
        (0, child_process_1.execSync)(waitCommand, { stdio: "inherit" });
        ...
        console.log("- To stop: lane down");
        console.log("- To view container logs: lane logs");
        console.log("- To follow container logs: lane logs -f");
        console.log("- To view system logs: lane logs --system");
        // Warn about turbo flag performance characteristics
        if (turbo && (profile === "stage" || profile === "stage-release")) {
            console.log("\n⚠️  Performance Note: --turbo flag enabled faster emulation");
            console.log("   This provides faster development experience but performance will NOT");
        ...
        if (hot && profile === "dev") {
            if (hasHotReloadSupport(imageTag)) {
                ...
                console.log(`📁 Watching for changes in: ${(0, process_1.cwd)()}`);
...
function buildDevContainer() {
    ...
    try {
        ...
        const imageName = "lane-dev-local";
        const buildCommand = `docker build -t ${imageName} .`;
        console.log(`Building image: ${imageName}`);
        console.log(`Executing: ${buildCommand}`);
        (0, child_process_1.execSync)(buildCommand, { stdio: "inherit", cwd: currentDir });
        return imageName;
    }
    catch (err) {
...
function startFileWatcher(imageTag, profile, cacheDir, useDepot = false, useTarContext = true, forceDockerTar = false, turbo = false, guestAgentImage, skipMessage = false, hot = false, useExistingImage) {
    const currentDir = (0, process_1.cwd)();
    if (!skipMessage) {
    ...
    const watcher = (0, fs_1.watch)(currentDir, { recursive: true }, (eventType, filename) => {
        if (!filename)
            return;
