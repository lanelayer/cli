...
exports.handlePushCommand = handlePushCommand;
...
function getCacheDirectory(imageTag, profile) {
...
function resolveRegistryPath(registryPath) {
    ...
    if (customRemoteMatch) {
        ...
        const remoteHash = (0, crypto_1.createHash)("sha256")
            .update(remoteKey)
            .digest("hex")
            .substring(0, 8);
        const remoteConfigPath = (0, path_1.join)((0, os_1.homedir)(), ".lane", "remotes", `${remoteHash}.json`);
        if ((0, fs_1.existsSync)(remoteConfigPath)) {
            try {
                const configContent = (0, fs_1.readFileSync)(remoteConfigPath, "utf8");
...
function handlePushCommand(args) {
    ...
    if (!resolvedRegistryPath.includes("/") ||
        !resolvedRegistryPath.includes(":")) {
    ...
    if (sourceOnly) {
        ...
        const contextBuilder = new tar_context_1.TarContextBuilder({
            contextPath: (0, process_1.cwd)(),
            outputPath: (0, path_1.join)(userCacheDir || getCacheDirectory(resolvedRegistryPath, "prod"), "source-context.tar"),
            deterministic: true,
        ...
        try {
            ...
            try {
                (0, child_process_1.execSync)(`curl -X PUT --data-binary @${tarPath} "${sourceUrl}"`, {
                    stdio: "inherit",
                ...
                if (resolved.config?.notify_url) {
                    try {
                        ...
                        const response = (0, child_process_1.execSync)(`curl -X POST -H "Content-Type: application/json" -d '${JSON.stringify(notificationData)}' "${resolved.config.notify_url}"`, {
                            encoding: "utf8",
                            stdio: "pipe",
                        });
    ...
    if (gitOnly) {
        ...
        try {
            (0, child_process_1.execSync)("git status", { stdio: "pipe" });
        }
        catch (err) {
        ...
        try {
            ...
            try {
                (0, child_process_1.execSync)(`git remote get-url ${remoteName}`, { stdio: "pipe" });
                console.log(`✅ Git remote '${remoteName}' already exists`);
            }
            catch (err) {
                console.log(`🔗 Adding git remote '${remoteName}': ${resolved.config.git}`);
                (0, child_process_1.execSync)(`git remote add ${remoteName} ${resolved.config.git}`, {
                    stdio: "inherit",
                });
            }
            ...
            try {
                (0, child_process_1.execSync)(`git push ${remoteName} ${currentBranch}`, {
                    stdio: "inherit",
                });
    ...
    if (!(0, fs_1.existsSync)((0, path_1.join)((0, process_1.cwd)(), "Dockerfile"))) {
        console.error("Error: No Dockerfile found in current directory");
        process.exit(1);
    }
    console.log(`🚀 Building and pushing prod container to: ${resolvedRegistryPath}`);
    ...
    if (useDepot) {
        ...
        const buildArgs = [
            ...
            "--sbom=false",
            "--build-arg",
            "SOURCE_DATE_EPOCH=1752444000",
            "--output",
            `type=registry,name=${resolvedRegistryPath}`,
            "--metadata-file",
            (0, path_1.join)(cacheDir, "build-metadata.json"),
        ];
    ...
    try {
        ...
        let digest = "";
        try {
            const metadataPath = (0, path_1.join)(cacheDir, "build-metadata.json");
            if ((0, fs_1.existsSync)(metadataPath)) {
                const metadataContent = (0, fs_1.readFileSync)(metadataPath, "utf8");
