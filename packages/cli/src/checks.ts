...
export function checkDockerAvailable() {
  try {
    ...
    console.error(
      "Error: Docker is not available or not running. Please install/start Docker and try again."
    );
    process.exit(1);
  }
}
...
export function checkVsockSupport() {
  ...
  try {
    ...
    const result = execSync(
      "docker run --rm --privileged alpine:latest ls -la /dev/vsock",
      {
        encoding: "utf8",
        stdio: "pipe",
      }
    ...
    if (result.includes("/dev/vsock")) {
      ...
      console.error(
        "vsock support is required for LaneLayer to function properly."
      );
      console.error(
        "Please ensure your system supports vsock or install the necessary kernel modules."
      );
      process.exit(1);
    }
  } catch (err) {
...
export function checkBuildxAvailable() {
  try {
    ...
    console.error(
      "Error: Docker buildx is not available. Please install Docker buildx and try again."
    );
    process.exit(1);
  }
}

export function checkLaneBuilder() {
...
export function checkOciExportSupport() {
  try {
    ...
    if (currentBuilderInfo.includes("Driver: docker")) {
      console.log(
        "⚠️  Current buildx builder uses docker driver, which does not support OCI exports"
      );
      console.log(
        "   This will cause issues with stage/prod profile builds that use OCI output format."
      );
      console.log("");
      ...
      for (const line of lines) {
        if (
          line.includes("docker-container") ||
          line.includes("kubernetes") ||
      ...
      if (compatibleBuilders.length > 0) {
        ...
        console.log(
          "💡 To fix this, create a new builder with docker-container driver:"
        );
        console.log(
          "   docker buildx create --name oci-builder --driver docker-container --use"
        );
        console.log("");
    ...
    console.error(
      "Please ensure you have a working buildx builder configured."
    );
    process.exit(1);
  }
}
...
export function requireOciExportSupport() {
  try {
    ...
    if (currentBuilderInfo.includes("Driver: docker")) {
      console.error(
        "❌ Error: Current buildx builder uses docker driver, which does not support OCI exports"
      );
      console.error(
        "   This is required for stage/prod profile builds that use OCI output format."
      );
      console.error("");
...
export function checkRiscv64Support() {
  try {
    ...
    execSync("docker run --rm --platform linux/riscv64 hello-world:latest", {
      stdio: "pipe",
    });
  } catch (err) {
    console.log(
      "⚠️  RISC-V 64-bit binary execution not supported. Installing binfmt emulation..."
    );
    try {
      execSync(
      ...
      console.error(
        "Please run manually: docker run --privileged --rm tonistiigi/binfmt --install riscv64"
      );
      process.exit(1);
    }
  }
