    ...
    if (existsSync(composePath)) {
      ...
      if (systemMode) {
        ...
        if (
          ...
          execSync(`docker logs${followFlag} ${containerName}`, {
            stdio: "inherit",
          });
        } else {
          // Show Docker Compose logs for dev
          const followFlag = followMode ? " -f" : "";
          execSync(`docker compose -f ${composePath} logs${followFlag}`, {
            stdio: "inherit",
          });
        }
      } else {
        ...
        if (
          ...
          try {
            execSync(
              `docker exec ${containerName} ssh -o StrictHostKeyChecking=no -i /work/ssh.debug-key -p 8022 localhost "${logCommand}"`,
              { stdio: "inherit" }
            );
          } catch (sshErr) {
          ...
          const followFlag = followMode ? " -f" : "";
          execSync(
            `docker compose -f ${composePath} logs${followFlag} isolated_service`,
            { stdio: "inherit" }
          );
        }
      }
    ...
    if (existsSync(composePath)) {
      ...
      if (systemMode) {
        ...
        if (
          ...
          console.log(
            `Detected ${profile} profile (system mode) - executing command in VM...`
          );
          try {
            execSync(
              `docker exec ${containerName} ssh -o StrictHostKeyChecking=no -i /work/ssh.debug-key -p 8022 localhost "${command}"`,
              { stdio: "inherit" }
            );
          } catch (sshErr) {
            // SSH command execution errors should be treated as command errors
          ...
          console.log(
            "Detected dev profile (system mode) - executing command in container..."
          );
          execSync(
            `docker compose -f ${composePath} exec isolated_service ${command}`,
            { stdio: "inherit" }
          );
        }
      } else {
        ...
        if (
          ...
          console.log(
            `Detected ${profile} profile - executing command in container...`
          );
          try {
            execSync(
              `docker exec ${containerName} ssh -o StrictHostKeyChecking=no -i /work/ssh.debug-key -p 8022 localhost "ctr -n services.linuxkit task exec --exec-id debug app ${command}"`,
              { stdio: "inherit" }
            );
          } catch (sshErr) {
            // SSH command execution errors should be treated as command errors
          ...
          console.log(
            "Detected dev profile - executing command in container..."
          );
          execSync(
            `docker compose -f ${composePath} exec isolated_service ${command}`,
            { stdio: "inherit" }
    ...
    if (existsSync(composePath)) {
      ...
      if (systemMode) {
        ...
        if (
          ...
          try {
            execSync(
              `docker exec -it ${containerName} ssh -t -o StrictHostKeyChecking=no -i /work/ssh.debug-key -p 8022 localhost`,
              { stdio: "inherit" }
            );
          } catch (sshErr) {
          ...
          console.log(
            "Detected dev profile (system mode) - opening shell in container..."
          );
          console.log('Type "exit" to return to your host shell');
          execSync(
            `docker compose -f ${composePath} exec isolated_service /bin/sh`,
            { stdio: "inherit" }
          );
        }
      } else {
        ...
        if (
          ...
          console.log(
            "Detected stage/prod profile - connecting to container..."
          );
          console.log('Type "exit" to return to your host shell');

          try {
            execSync(
              `docker exec -it ${containerName} ssh -t -o StrictHostKeyChecking=no -i /work/ssh.debug-key -p 8022 localhost "ctr -n services.linuxkit task exec --exec-id debug --tty app /bin/sh"`,
              { stdio: "inherit" }
            );
          } catch (sshErr) {
            // SSH command execution errors should be treated as command errors
  ...
  try {
    ...
    if (existsSync(composePath)) {
      ...
      if (
        ...
        console.log(
          `Detected ${profile} profile - viewing file in container...`
        );
        try {
          execSync(
            `docker exec ${containerName} ssh -o StrictHostKeyChecking=no -i /work/ssh.debug-key -p 8022 localhost "ctr -n services.linuxkit task exec --exec-id debug app cat ${filePath}"`,
            { stdio: "inherit" }
          );
        } catch (sshErr) {
          // SSH command execution errors should be treated as command errors
        ...
        execSync(
          `docker compose -f ${composePath} exec isolated_service cat ${filePath}`,
          { stdio: "inherit" }
        );
      }
    } else {
...
export function handlePerfCommand(args: string[]): void {
  ...
  if (profile !== "stage" && profile !== "prod-debug") {
    console.error(
      "lane perf is only available for stage and prod-debug profiles."
    );
    process.exit(1);
  }
  ...
  const useTty =
    subcommand === "top" || subcommand === "report" || subcommand === "record";
  ...
  try {
    ...
    if (existsSync(composePath)) {
      ...
      try {
        execSync(
          `${dockerExec} ${containerName} ssh ${sshFlags} -o StrictHostKeyChecking=no -i /work/ssh.debug-key -p 8022 localhost "${fullCmd}"`,
          { stdio: "inherit" }
        );
      } catch (err) {
        if (subcommand === "record") {
