...
function resolveRegistryPath(registryPath: string): {
  resolvedPath: string;
  config?: any;
  remoteKey?: string;
  path?: string;
    ...
    if (existsSync(remoteConfigPath)) {
      try {
        ...
        if (config.registry_url) {
          ...
          console.log(
            `🔗 Resolved remote ${remoteKey} to registry: ${config.registry_url}`
          );
          console.log(`📤 Final registry path: ${resolvedPath}`);
          return { resolvedPath, config, remoteKey, path };
        } else {
          console.warn(
            `⚠️  Remote config ${remoteConfigPath} missing 'registry_url' field`
          );
        }
      } catch (err) {
        console.warn(
          `⚠️  Failed to read remote config ${remoteConfigPath}:`,
          err
        );
      }
    } else {
      console.log(
        `ℹ️  No remote config found for ${remoteKey} (expected: ${remoteConfigPath})`
      );
      console.log(
        `💡 Create a config file with: {"registry_url": "your-registry.com", "git": "git@github.com:user/repo.git", "remote_name": "optional-custom-name", "notify_url": "https://webhook.example.com"}`
      );
    }
  }
...
export function handlePushCommand(args: string[]): void {
  ...
  if (
    ...
    console.error(
      "Error: Registry path must be in format: registry.com/name:tag"
    );
  ...
  if (sourceOnly) {
    if (!resolved.config || !resolved.config.source) {
      console.error(
        'Error: --source flag requires a custom remote with "source" endpoint configured'
      );
      console.error(
        'Add "source": "your-source-endpoint.com" to your remote config file'
      );
      process.exit(1);
    }

    console.log(
      `📤 Pushing source context to: ${resolved.config.source}/${resolved.path}`
    );
    ...
    const contextBuilder = new TarContextBuilder({
      contextPath: cwd(),
      outputPath: join(
        userCacheDir || getCacheDirectory(resolvedRegistryPath, "prod"),
        "source-context.tar"
      ),
      deterministic: true,
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "Warning: Could not clean up source context tar file:",
          cleanupErr
        );
      }
    }
  ...
  if (gitOnly) {
    if (!resolved.config || !resolved.config.git) {
      console.error(
        'Error: --git flag requires a custom remote with "git" endpoint configured'
      );
      console.error(
        'Add "git": "git@github.com:user/repo.git" to your remote config file'
      );
      console.error(
        'Optional: Add "remote_name": "custom-name" to use a different git remote name'
      );
      process.exit(1);
    }
    ...
    try {
      ...
      try {
        ...
        console.log(
          `🔗 Adding git remote '${remoteName}': ${resolved.config.git}`
        );
        execSync(`git remote add ${remoteName} ${resolved.config.git}`, {
          stdio: "inherit",
        });
      }
      ...
      try {
        execSync(`git push ${remoteName} ${currentBranch}`, {
          stdio: "inherit",
        });
        console.log(
          `✅ Successfully pushed to git remote: ${resolved.config.git}`
        );
        ...
        console.error(
          "💡 Make sure you have access to the git repository and have commits to push"
        );
        process.exit(1);
      }
    } catch (err) {
  ...
  const cacheDir =
    userCacheDir || getCacheDirectory(resolvedRegistryPath, profile);
  ...
  if (useDepot) {
    ...
    const buildArgs = [
      ...
      "--metadata-file",
      join(cacheDir, "build-metadata.json"),
    ];
