...
function hasHotReloadSupport(imageTag: string): boolean {
  try {
    const label = execSync(
      `docker inspect ${imageTag} --format '{{index .Config.Labels "lane.hot-reload"}}'`,
      {
        encoding: "utf8",
...
function resolvePlatforms(profile: string): string[] {
  switch (profile) {
    ...
    default:
      console.error(
        `Error: Unknown profile '${profile}'. Valid profiles: dev, stage, stage-release, prod, prod-debug`
      );
      process.exit(1);
  }
}
...
function buildLinuxKitImage(
  ...
  const imageName = process.env.LINUXKIT_IMAGE || LANE_SNAPSHOT_BUILDER_IMAGE;
  ...
  if (existsSync(dockerConfigPath)) {
    try {
      ...
      if (config.credsStore === "desktop") {
        console.log(
          `Docker config found but using 'desktop' credential store - skipping config mount (credentials in macOS keychain)`
        );
      } else {
        dockerConfigMount = [
          "-v",
    ...
    console.log(
      `No Docker config found at ${dockerConfigPath} - authentication may not work for private registries`
    );
  }
  ...
  try {
    ...
    const vcTarPath = cacheDir
      ? join(cacheDir, `vc${debugSuffix}.tar`)
      : join(currentDir, `vc${debugSuffix}.tar`);
    const vcSquashfsPath = cacheDir
      ? join(cacheDir, `vc${debugSuffix}.squashfs`)
      : join(currentDir, `vc${debugSuffix}.squashfs`);

    if (!forceRebuild && existsSync(vcSquashfsPath)) {
      console.log(
        `✅ vc${debugSuffix}.squashfs already exists, skipping LinuxKit build and squashfs creation`
      );
    } else {
      ...
      if (ociTarPath && existsSync(ociTarPath)) {
        ...
        console.log(
          "ℹ️  No OCI tar provided - using image reference directly in LinuxKit YAML"
        );
      }
      ...
      try {
        if (existsSync(vcTarPath)) {
          const vcTarHash = execSync(`sha256sum "${vcTarPath}"`, {
            encoding: "utf8",
          })
            .trim()
            .split(" ")[0];
          ...
          console.log(
            `⚠️  vc${debugSuffix}.tar not found after LinuxKit build`
          );
        }
      } catch (err) {
        console.log(
          `⚠️  Could not calculate vc${debugSuffix}.tar SHA256:`,
          err
        );
      }
      ...
      const result = spawnSync(snapshotCommand[0], snapshotCommand.slice(1), {
      ...
      if (result.status !== 0) {
        ...
        throw new Error(
          `Command failed with status ${
            result.status || "null (process killed)"
          }`
        );
      }
    ...
    if (profile === "prod" || profile === "prod-debug") {
      ...
      const cmSquashfsPath = cacheDir
        ? join(cacheDir, `vc-cm-snapshot${debugSuffix}.squashfs`)
        : join(currentDir, `vc-cm-snapshot${debugSuffix}.squashfs`);
      const verityPath = cacheDir
        ? join(cacheDir, `vc-cm-snapshot${debugSuffix}.squashfs.verity`)
        : join(currentDir, `vc-cm-snapshot${debugSuffix}.squashfs.verity`);
      const rootHashPath = cacheDir
        ? join(cacheDir, `vc-cm-snapshot${debugSuffix}.squashfs.root-hash`)
        : join(currentDir, `vc-cm-snapshot${debugSuffix}.squashfs.root-hash`);

      // Check if we need to create Cartesi machine snapshot
      if (!forceRebuild && existsSync(cmSnapshotPath)) {
        console.log(
          `✅ vc-cm-snapshot${debugSuffix} already exists, skipping Cartesi machine creation`
        );
      } else {
        if (forceRebuild && existsSync(cmSnapshotPath)) {
          console.log(
            `🔄 Force rebuild: removing existing vc-cm-snapshot${debugSuffix}`
          );
          execSync(`rm -rf "${cmSnapshotPath}"`, { stdio: "ignore" });
        }

        console.log(
          `Creating Cartesi machine snapshot for ${profile} profile...`
        );
        ...
        const cartesiResult = spawnSync(
          cartesiCommand[0],
          cartesiCommand.slice(1),
          { stdio: "inherit", cwd: currentDir }
        );

        if (cartesiResult.status !== 0) {
          console.error(
            "Cartesi machine command failed with status:",
            cartesiResult.status
          );
          throw new Error(
            `Cartesi machine command failed with status ${cartesiResult.status}`
          );
        }
        ...
        try {
          ...
          if (existsSync(hashPath)) {
            ...
            console.error(
              `❌ Error: Cartesi machine hash file not found at vc-cm-snapshot${debugSuffix}/hash`
            );
            console.error(
              "This indicates the Cartesi machine creation failed or the hash file was not generated."
            );
            console.error(
              "Checking if snapshot directory exists and listing contents..."
            );
            try {
              if (existsSync(cmSnapshotPath)) {
                const contents = execSync(`ls -la "${cmSnapshotPath}"`, {
                  encoding: "utf8",
                });
              ...
              console.error(
                "Could not list snapshot directory contents:",
                listErr
              );
            }
            process.exit(1);
          ...
          console.error(
            "❌ Error: Could not read Cartesi machine hash:",
            hashErr
          );
          process.exit(1);
        }
      ...
      if (!forceRebuild && existsSync(cmSquashfsPath)) {
        console.log(
          `✅ vc-cm-snapshot${debugSuffix}.squashfs already exists, skipping compression`
        );
      } else {
        console.log(
          `Creating compressed Cartesi machine snapshot for ${profile} profile...`
        );
        ...
        const compressResult = spawnSync(
          compressCommand[0],
          compressCommand.slice(1),
          { stdio: ["inherit", "pipe", "pipe"], cwd: currentDir }
        );
        ...
        if (compressResult.status !== 0) {
          ...
          throw new Error(
            `Compression command failed with status ${
              compressResult.status || "null (process killed)"
            }`
          );
        }

        console.log(
          `✅ Compressed Cartesi machine snapshot created successfully for ${profile} profile`
        );
      }
      ...
      try {
        ...
        if (fileSize % 512 !== 0) {
          console.error(
            `❌ Error: vc-cm-snapshot${debugSuffix}.squashfs size (${fileSize} bytes) is not divisible by 512`
          );
        ...
        console.error(
          `❌ Error: Could not verify vc-cm-snapshot${debugSuffix}.squashfs file size:`,
          err
        );
        process.exit(1);
      }
      ...
      if (!forceRebuild && existsSync(verityPath) && existsSync(rootHashPath)) {
        ...
        try {
          ...
          if (existsSync(hashPath)) {
            ...
            console.log(
              `Using Cartesi machine hash for verity: ${cartesiMachineHash}`
            );
          } else {
        ...
        const deterministicUuid = `${uuidBase.substring(
          0,
          8
        )}-${uuidBase.substring(8, 12)}-${uuidBase.substring(
          12,
          16
        )}-${uuidBase.substring(16, 20)}-${uuidBase.substring(20, 32)}`;
        ...
        const verityResult = spawnSync(
          verityCommand[0],
          verityCommand.slice(1),
          { stdio: ["inherit", "pipe", "pipe"], cwd: currentDir }
        );
        ...
        if (verityResult.status !== 0) {
          ...
          throw new Error(
            `Verity setup command failed with status ${verityResult.status}`
          );
        }
        ...
        if (verifyResult.status !== 0) {
          ...
          throw new Error(
            `Verity verification failed with status ${verifyResult.status}`
          );
        }
      ...
      try {
        ...
        if (existsSync(vcSquashfsPath)) {
          const vcSquashfsHash = execSync(`sha256sum "${vcSquashfsPath}"`, {
            encoding: "utf8",
          })
            .trim()
            .split(" ")[0];
        ...
        console.log(
          `⚠️  Could not calculate vc${debugSuffix}.squashfs SHA256:`,
          err
        );
      }
      ...
      try {
        ...
        if (existsSync(cmSquashfsPath)) {
          const cmSquashfsHash = execSync(`sha256sum "${cmSquashfsPath}"`, {
            encoding: "utf8",
          })
            .trim()
            .split(" ")[0];
          console.log(
            `📦 vc-cm-snapshot${debugSuffix}.squashfs SHA256: ${cmSquashfsHash}`
          );
        } else {
        ...
        console.log(
          `⚠️  Could not calculate vc-cm-snapshot${debugSuffix}.squashfs SHA256:`,
          err
        );
      }
      ...
      try {
        if (existsSync(rootHashPath)) {
          const rootHash = execSync(`cat "${rootHashPath}"`, {
            encoding: "utf8",
          }).trim();
...
export function buildImage(
  ...
  if (useExistingImage) {
    ...
    if (
      ...
      console.log(
        "✅ Using existing image for stage/prod profile - will reference directly in LinuxKit YAML"
      );
      ...
      const yamlPath = generateLinuxKitYaml(
        imageTag,
        profile,
        cacheDir,
        undefined,
  ...
  if (useDepot) {
    ...
    try {
      ...
      console.log(
        "💡 Tip: depot binary found in PATH. Use --depot for faster builds!"
      );
    } catch (err) {
  ...
  if ((profile === "prod" || profile === "prod-debug") && guestAgentImage) {
    ...
    if (existsSync(guestAgentCacheFile)) {
      ...
      if (cachedImage !== guestAgentImage) {
        console.log(
          `🔄 Guest agent image changed: ${cachedImage} → ${guestAgentImage}`
        );
  ...
  if (useDepot) {
    ...
    if (profile === "dev") {
      ...
      buildArgs.push(
        "--output",
        `type=oci,dest=${ociTarPath},name=${imageTag},rewrite-timestamp=true`
      );
    }
  ...
  try {
    ...
    if (useDepot && profile === "dev") {
      ...
      if (profile === "dev") {
        ...
        console.log(
          `Docker image not loaded (${profile} profile uses OCI import)`
        );
      }
    }
    console.log(`Cache directory: ${cacheDir}`);
    ...
    if (!turbo && (profile === "stage" || profile === "stage-release")) {
      ...
      console.log(
        "   If you need even faster emulation, try the --turbo flag for multi-core QEMU."
      );
    }
    ...
    if (profile === "dev") {
      ...
      console.log(
        "   This provides maximum performance but runs on your native architecture"
      );
    ...
    if (contextBuilder) {
      try {
        ...
        console.warn(
          "Warning: Could not clean up context tar file:",
          cleanupErr
        );
      }
    }
...
export function runDevEnvironment(
  imageTag: string,
  profile: string,
  cacheDir?: string,
  forceRebuild = false,
  ...
  try {
    ...
    try {
      ...
      const containersUsing8080 = allContainers
        ...
        .filter(
          (line) =>
            line.includes(":8080") && !line.startsWith(`${pathHash}-lane-`)
        );

      if (containersUsing8080.length > 0) {
        console.error(
          "❌ Error: Port 8080 is already in use by another container"
        );
        containersUsing8080.forEach((container) =>
          console.error(`   ${container}`)
        );
        console.error("");
        console.error(
          "Please stop the conflicting container or use a different port."
        );
        console.error(
          "You can stop all LaneLayer environments with: lane prune --local"
        );
        process.exit(1);
      }
    } catch (err) {
    ...
    if (profileChangeNeeded && currentProfile) {
      ...
      console.log(
        `✅ Current profile (${currentProfile}) matches requested profile (${profile})`
      );
    } else {
      ...
      try {
        ...
        if (currentImage) {
          ...
          if (currentImage !== expectedImage) {
            console.log(
              `🔄 Image tag changed from ${currentImage} to ${expectedImage}`
            );
            needsUpdate = true;
          } else {
        ...
        console.log(
          "⚠️  Could not read existing compose file, will regenerate"
        );
        needsUpdate = true;
      }
    } else {
    ...
    if (needsUpdate) {
      ...
      const buildCacheDir = getCacheDirectory(
        imageTag,
        profile,
        guestAgentImage
      );
    ...
    if (needsUpdate) {
      ...
      const containersExist =
        ...
        execSync(
          `docker compose -f ${composePath} ps --services --filter "status=running"`,
          { encoding: "utf8" }
        ).trim().length > 0;
      ...
      if (containersExist) {
        ...
        execSync(
          `docker compose -f ${composePath} up -d --force-recreate --wait isolated_service`,
          { stdio: "inherit" }
        );
      } else {
    ...
    if (profile === "dev") {
      console.log(
        `- Function endpoint: http://localhost:8080 (IPv4) or http://[::1]:8081 (IPv6)`
      );
    } else {
    ...
    if (hot && profile === "dev") {
      if (hasHotReloadSupport(imageTag)) {
        ...
        console.log(
          `📁 Source code mounted to /app - changes will trigger instant restarts\n`
        );
      } else {
        console.log(
      ...
      if (useExistingImage) {
        ...
        if (profile === "dev") {
          ...
          runDevEnvironment(
            ...
            guestAgentImage,
            false
          ); // hot=false to avoid infinite recursion
          console.log(`✅ Environment rebuilt and restarted successfully!`);
        }
      }
    } catch (err) {
